...
public class Monitor : MonoBehaviour
{
    ...
    /// <summary>
    /// This float represents how high above the target the monitors will be.
    /// </summary>
    [HideInInspector]
        ...
        Transform target = null)
    {
        ...
        if (!displayValues.ContainsKey(key))
        {
            var dv = new DisplayValue
            {
                ...
                value = value,
                monitorDisplayType = displayType
            };
            displayValues[key] = dv;
            while (displayValues.Count > 20)
            {
                string max = displayValues
                    .Aggregate(
                        (l, r) => l.Value.time < r.Value.time ? l : r)
                    .Key;
    ...
    void OnGUI()
    {
        ...
        foreach (Transform target in toIterate)
        {
            ...
            if (!(target == canvas.transform))
            {
                ...
                origin = new Vector3(
                    worldPosition.x - keyPixelWidth * scale,
            ...
            foreach (string key in displayValues.Keys.OrderBy(x => -displayValues[x].time))
            {
                ...
                GUI.Label(
                    ...
                    key,
                    keyStyle);
                switch (displayValues[key].monitorDisplayType)
                {
                    case MonitorType.text:
                    ...
                    case MonitorType.slider:
                        ...
                        GUIStyle sliderStyle = greenStyle;
                        if (sliderValue < 0)
                        {
                            sliderValue = Mathf.Min(1f, -sliderValue);
                            sliderStyle = redStyle;
                        }
                        GUI.Box(new Rect(
                                origin.x + paddingwidth + keyPixelWidth,
                                origin.y - (index + 0.9f) * keyPixelHeight,
                                keyPixelWidth * sliderValue, keyPixelHeight * 0.8f),
                            GUIContent.none, sliderStyle);
                        break;
                    case MonitorType.hist:
                        float histWidth = 0.15f;
                        float[] histVals = ToFloatArray(displayValues[key].value);
                        for (int i = 0; i < histVals.Length; i++)
                        {
                            float value = Mathf.Min(histVals[i], 1);
                            GUIStyle histStyle = greenStyle;
                            if (value < 0)
                            {
                            ...
                            GUI.Box(new Rect(
                                    ...
                                    GUIContent.none, histStyle);
                        }

                        break;
                    case MonitorType.bar:
                        float[] barVals = ToFloatArray(displayValues[key].value);
                        float valsSum = 0f;
                        float valsCum = 0f;
                        foreach (float f in barVals)
                        {
                            valsSum += Mathf.Max(f, 0);
                        }
                        if (System.Math.Abs(valsSum) < 1e-10)
                        {
                        ...
                        else
                        {
                            ...
                            {
                                float value = Mathf.Max(barVals[i], 0) / valsSum;
                                GUI.Box(new Rect(
